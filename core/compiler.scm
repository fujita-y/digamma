...
(define (codegen expr env next-reg tail? ctx)
  (cond ((symbol? expr) (codegen-symbol expr env tail? ctx))
        ...
        ((eq? (car expr) 'quote)
         ...
         (if tail? (compiler-ctx-emit! ctx `(ret))))
        ((and (eq? (car expr) 'unspecified) (null? (cdr expr)))
         (compiler-ctx-emit! ctx `(const r0 ,(unspecified)))
         (if tail? (compiler-ctx-emit! ctx `(ret))))
        ((eq? (car expr) 'begin)
         (if (null? (cdr expr))
...
(define (codegen-application expr env next-reg tail? ctx)
  (if (and (pair? (car expr)) (match-rec-pattern (car expr)))
      (let* ((let-expr (car expr)) (args (cdr expr)) (num-args (length args)) (bindings (cadr let-expr)) (name (caar bindings)) (sym (cadar bindings))
        ...
        (codegen-lambda lambda-expr (cons (list (cons name 'self)) env) next-reg #f ctx)
