...
(define (move-args num-args base ctx)
  (let move-loop ((j 0))
    (if (< j num-args)
        (begin
          (let ((dst (make-reg j)) (src (make-reg (+ base j))))
            (unless (eq? dst src) (compiler-ctx-emit! ctx `(mov ,dst ,src))))
          (move-loop (+ j 1))))))
...
(define (analyze-forward-refs vars vals)
  (let ((ht (make-eq-hashtable))
        (pos-ht (make-eq-hashtable)))
    (let index ((vs vars) (i 0))
      (unless (null? vs) (hashtable-set! pos-ht (car vs) i) (index (cdr vs) (+ i 1))))
          ...
          (begin
            (if (and (pair? (car vs)) (eq? (car (car vs)) 'lambda))
                (let ((fvs (analyze-free-vars-compiler (car vs) '())))
                  (for-each (lambda (fv)
                              (let ((pos (hashtable-ref pos-ht fv #f)))
                                (if (and pos (>= pos i))
