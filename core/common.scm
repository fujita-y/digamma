...
;; Matches: (let ((<name> #f)) (set! <name> (lambda ...)) ...)
(define (match-rec-pattern expr)
  (define (captured-by-lambda? name expr depth)
    (cond ((pair? expr)
           (let ((d (if (eq? (car expr) 'lambda) (+ depth 1) depth)))
             (or (captured-by-lambda? name (car expr) d)
                 (captured-by-lambda? name (cdr expr) d))))
          (else (and (eq? expr name) (> depth 0)))))
  (define (tail-call-only? name exprs)
    (define (check-expr e tail?)
      (cond ((pair? e)
             (case (car e)
               ((begin) (check-seq (cdr e) tail?))
               ((if) (and (check-expr (cadr e) #f)
                          (check-expr (caddr e) tail?)
                          (if (pair? (cdddr e))
                              (check-expr (cadddr e) tail?)
                              #t)))
               ((let)
                (and (every? (lambda (b)
                               (and (pair? b)
                                    ...
                                    (check-expr (cadr b) #f)))
                             (cadr e))
                     (check-seq (cddr e) tail?)))
               ((set!)
                (check-expr (caddr e) #f))
               ((lambda quote) #t)
               (else
                (if tail?
                    (if (eq? (car e) name)
                        (every? (lambda (arg) (check-expr arg #f)) (cdr e))
                        (and (check-expr (car e) #f)
                             (every? (lambda (arg) (check-expr arg #f)) (cdr e))))
                    (every? (lambda (x) (check-expr x #f)) e)))))
            (else
             (not (eq? e name)))))
    (define (check-seq seq tail?)
      (cond ((null? seq) #t)
            ((null? (cdr seq)) (check-expr (car seq) tail?))
            (else (and (check-expr (car seq) #f)
                            ...
                            (let ((stmt1 (car body)))
                              (and (pair? stmt1)
                                   ...
                                   (let ((rhs (caddr stmt1)))
                                     (and (pair? rhs)
                                          (eq? (car rhs) 'lambda)
                                          (not (captured-by-lambda? name (cddr rhs) 0))
                                          (tail-call-only? name (cddr rhs))))))))))))))
