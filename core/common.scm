...
(define (match-rec-pattern expr)
  (letrec ((name-in-nested-lambda?
            (lambda (name expr depth)
              (cond ((pair? expr)
                     (if (eq? (car expr) 'lambda)
                         (or (name-in-nested-lambda? name (car expr) depth)
                             (name-in-nested-lambda? name (cdr expr) (+ depth 1)))
                         (or (name-in-nested-lambda? name (car expr) depth)
                             (name-in-nested-lambda? name (cdr expr) depth))))
                    ((eq? expr name) (> depth 0))
                    (else #f)))))
    (let ((match?
           (lambda (expr)
             (and (pair? expr)
