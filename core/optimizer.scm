...
(for-each (lambda (p) (hashtable-set! pure-primitives-ht p #t)) pure-primitives)
(define (pure-primitive? sym) (hashtable-contains? pure-primitives-ht sym))
...
(for-each (lambda (p) (hashtable-set! structural-mutation-ht p #t)) structural-mutation-primitives)
(define (structural-mutation-primitive? sym) (hashtable-contains? structural-mutation-ht sym))
      ...
      (if (null? l)
          ...
          (let ((x (car l)))
            (if (hashtable-contains? ht x)
                (loop (cdr l) acc)
                (begin (hashtable-set! ht x #t)
                       (loop (cdr l) (cons x acc)))))))))
  ...
  (let ((usage (make-eq-hashtable))
    ...
    (let walk ((e body) (lambda-depth 0) (s shadowed) (lbound '()))
      (cond ((symbol? e)
            ...
            ((and (pair? e) (structural-mutation-primitive? (car e)))
             (let ((target (cadr e)))
               (when (symbol? target)
  ...
  (cond ((symbol? expr) #f)
        ...
        ((eq? (car expr) 'letrec*)
         (or (any (lambda (b) (has-effects? (cadr b))) (cadr expr))
             (any has-effects? (cddr expr))))
        (else
         (not (and (pure-primitive? (car expr))
...
(define (safe-to-inline-val? var val body-usage bound-vars mutated-vars)
  (let ((fvars (analyze-free-vars-optimizer val '())))
    (and (not (any (lambda (v) (memq v mutated-vars)) fvars))
         (or (every (lambda (v) (or (memq v bound-vars) (pure-primitive? v))) fvars)
...
(define (substitute-many expr mapping)
  (if (null? mapping)
      expr
      ;; Pre-compute free vars for each mapping entry once
      (let ((mapping-fvs (map (lambda (p) (cons p (analyze-free-vars-optimizer (cdr p) '()))) mapping)))
        (let recur ((expr expr) (mfvs mapping-fvs))
          (let ((mapping (map car mfvs)))
            (cond ((symbol? expr) (cond ((assq expr mapping) => cdr) (else expr)))
                  ...
                  ((eq? (car expr) 'lambda)
                   ...
                   (let* ((params (flatten-params-opt (cadr expr)))
                          (new-mfvs (filter (lambda (pf)
                                             (and (not (memq (caar pf) params))
                                                  (not (any (lambda (fv) (memq fv params)) (cdr pf)))))
                                           mfvs)))
                     `(lambda ,(cadr expr) ,@(map (lambda (e) (recur e new-mfvs)) (cddr expr)))))
                  ((eq? (car expr) 'let)
                   (let* ((bindings (cadr expr))
                          (vars (map car bindings))
                          (new-mfvs (filter (lambda (pf)
                                             (and (not (memq (caar pf) vars))
                                                  (not (any (lambda (fv) (memq fv vars)) (cdr pf)))))
                                           mfvs)))
                     `(let ,(map (lambda (b) (list (car b) (recur (cadr b) mfvs))) bindings)
                        ,@(map (lambda (e) (recur e new-mfvs)) (cddr expr)))))
                  ((eq? (car expr) 'letrec*)
                   (let* ((bindings (cadr expr))
                          (vars (map car bindings))
                          (new-mfvs (filter (lambda (pf)
                                             (and (not (memq (caar pf) vars))
                                                  (not (any (lambda (fv) (memq fv vars)) (cdr pf)))))
                                           mfvs)))
                     `(letrec* ,(map (lambda (b) (list (car b) (recur (cadr b) new-mfvs))) bindings)
                        ,@(map (lambda (e) (recur e new-mfvs)) (cddr expr)))))
                  (else (map (lambda (e) (recur e mfvs)) expr))))))))
...
(define (opt-let-inner bindings body bound-vars)
  (let ((floated '()) (main '()))
                  ...
                  (if (and (pair? val) (eq? (car val) 'let))
                      (begin (set! floated (append floated (cadr val)))
                             (let* ((inner-body (cddr val))
                                    (len (length inner-body))
                                    (last-expr  (if (null? inner-body) '() (list-ref inner-body (- len 1))))
                                    (mid-exprs  (if (<= len 1) '() (list-head inner-body (- len 1)))))
    ...
    (if (not (null? floated))
        ...
        (let-values (((mutated-list usage) (analyze-body-combined (make-seq body) '())))
          (let ((mutated-ht (make-eq-hashtable))
                (new-bind '()) (subst '()))
            (for-each (lambda (v) (hashtable-set! mutated-ht v #t)) mutated-list)
            (for-each (lambda (b)
                        (let* ((var (car b)) (val (cadr b)) (count (car (hashtable-ref usage var '(0 . #f)))))
                          (cond ((and (not (hashtable-contains? mutated-ht var)) (safe-to-inline-val? var val usage bound-vars mutated-list)
                                ...
                                ((and (not (hashtable-contains? mutated-ht var)) (pair? val) (eq? (car val) 'lambda) (should-inline? var val usage))
                                 (set! body (perform-inlining var val body))
                                 (if (has-effects? val) (set! new-bind (cons b new-bind))))
                                ((and (not (hashtable-contains? mutated-ht var)) (pair? val) (eq? (car val) 'lambda) (and (pair? (car body)) (eq? (car (car body)) 'if)))
                                 (let ((res (try-drop-lambda var val (car body))))
                                   (if (car res) (set! body (cons (cdr res) (cdr body))) (set! new-bind (cons b new-bind)))))
                                ((or (> count 0) (hashtable-contains? mutated-ht var) (has-effects? val))
...
(define (opt-begin expr bound-vars)
  (let ((flattened (let loop ((in (cdr expr)) (acc '()))
    ...
    (if (null? flattened) '(unspecified)
        (let* ((len (length flattened))
               (last (list-ref flattened (- len 1)))
               (init (list-head flattened (- len 1)))
               (filtered (filter has-effects? init)))
