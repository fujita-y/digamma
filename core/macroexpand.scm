...
(define (apply-import-spec spec bindings)
  (if (not (pair? spec))
      ...
      (case (car spec)
        ((only)
         (let ((inner (apply-import-spec (cadr spec) bindings))
               (ht (make-eq-hashtable)))
           (for-each (lambda (id) (hashtable-set! ht id #t)) (cddr spec))
           (filter (lambda (b) (hashtable-contains? ht (car b))) inner)))
        ((except)
         (let ((inner (apply-import-spec (cadr spec) bindings))
               (ht (make-eq-hashtable)))
           (for-each (lambda (id) (hashtable-set! ht id #t)) (cddr spec))
           (filter (lambda (b) (not (hashtable-contains? ht (car b)))) inner)))
        ((rename)
      ...
      (let* ((export-info (car record)) 
             ...
             (renamed-bindings 
               (map (lambda (b)
                    ...
                    bindings))
             (export-ht (make-eq-hashtable))
             (_ (for-each (lambda (e) (hashtable-set! export-ht e #t)) exports))
             (exported (filter (lambda (b) (hashtable-contains? export-ht (car b))) renamed-bindings)))
...
(define (expand-lambda-form expr m-env s-env r-env marks)
  (let* ((params (cadr expr)) (body (cddr expr))
         ...
         (new-r-env (fold-right (lambda (k v acc) (cons (cons k v) acc)) r-env p-names new-p-names))
...
(define (expand-let-form expr m-env s-env r-env marks)
  (if (symbol? (cadr expr))
      ...
      (let* ((bindings (cadr expr)) (body (cddr expr))
             ...
             (new-r-env (fold-right (lambda (k v acc) (cons (cons k v) acc)) r-env vars new-vars))
    ...
    (let loop ((decls decls) (exports '()) (imports '()) (body-forms '()))
      (if (null? decls)
          (let* ((export-info (process-export-specs (reverse exports)))
            ...
            (unspecified))
          (let ((decl (car decls)))
            (cond ((and (pair? decl) (eq? (car decl) 'export)) (loop (cdr decls) (append (cdr decl) exports) imports body-forms))
                  ((and (pair? decl) (eq? (car decl) 'import)) (loop (cdr decls) exports (append (cdr decl) imports) body-forms))
...
(define (partition-bindings bindings)
  (let loop ((bs bindings) (macros '()) (runtime '()))
    (if (null? bs)
        (cons (reverse macros) (reverse runtime))
        (if (macro-binding? (cdar bs))
            (loop (cdr bs) (cons (car bs) macros) runtime)
            (loop (cdr bs) macros (cons (car bs) runtime))))))

(define (eval-module-body imported-bindings body-forms m-env s-env r-env marks mod-name)
  (let* ((partitioned (partition-bindings imported-bindings))
         (macro-m (car partitioned))
         (runtime-i (cdr partitioned))
    ...
    (when (current-module-strict-mode)
      (let* ((refs (collect-referenced-ids (cons 'begin rt-forms)))
             (known-ht (make-eq-hashtable)))
        (for-each (lambda (ids) (for-each (lambda (id) (hashtable-set! known-ht id #t)) ids))
                  (list (map car imported-bindings) def-ids (map car macro-b) (map cdr internal-m)))
        (for-each
          (lambda (id)
            (unless (or (hashtable-contains? known-ht id)
...
(define current-macroexpand-depth (make-parameter 0))

(define (macroexpand expr . opt)
  (cond ((= (current-macroexpand-depth) 0)
         (set! *suffix-counter* 0) 
         (set! *mark-counter* 0)
         (hashtable-clear! *rename-env*)
         (hashtable-clear! *resolve-cache*)
         (with-parameter ((current-macroexpand-depth 1))
           (let ((res (expand expr '() '() '() '())))
             (if (and (pair? opt) (eq? (car opt) 'strip)) 
                 (strip-renames res) 
                 res))))
        (else
         (let ((saved-rename-alist (hashtable->alist *rename-env*))
               (saved-suffix *suffix-counter*)
               (saved-mark *mark-counter*))
           (hashtable-clear! *rename-env*)
           (hashtable-clear! *resolve-cache*)
           (with-parameter ((current-macroexpand-depth (+ (current-macroexpand-depth) 1)))
             (let ((res (expand expr '() '() '() '())))
               ;; Merge: restore saved renames, then overlay inner renames
               (let ((inner-alist (hashtable->alist *rename-env*)))
                 (hashtable-clear! *rename-env*)
                 (for-each (lambda (p) (hashtable-set! *rename-env* (car p) (cdr p))) saved-rename-alist)
                 (for-each (lambda (p) (hashtable-set! *rename-env* (car p) (cdr p))) inner-alist))
               (hashtable-clear! *resolve-cache*)
               (set! *suffix-counter* (max saved-suffix *suffix-counter*))
               (set! *mark-counter* (max saved-mark *mark-counter*))
               res))))))
