...
(define (inject-binding! name value)
  ...
  (eval `(define ,name *temp-value*) (interaction-environment)))
  ...
  (let recur ((clauses (cdr expr)))
    (if (null? clauses) '(begin)
        (let ((clause (car clauses)) (rest (cdr clauses)))
          (cond ((core-form? (car clause) 'else s-env) (expand (make-seq (cdr clause)) m-env s-env r-env))
                ...
                (else (expand `(if ,(car clause) ,(make-seq (cdr clause)) (cond ,@rest)) m-env s-env r-env)))))))
...
(define (eval-module-body imported-bindings body-forms m-env s-env r-env mod-name)
  (let* ((macro-m (filter (lambda (b) (macro-binding? (cdr b))) imported-bindings))
    ...
    (let* ((proc (eval wrapper (interaction-environment))) (vals (proc)) (rt-bindings (map cons def-ids vals)))
      (for-each (lambda (p val) (inject-binding! (cdr p) val)) internal-m vals)
      (append rt-bindings macro-b))))

(define (lookup-handler core-sym)
  (case core-sym
    ((define-syntax) expand-define-syntax) ((let-syntax) expand-let-syntax) ((letrec-syntax) expand-letrec-syntax)
    ((let*-syntax) expand-let*-syntax)
    ((lambda) expand-lambda) ((let) expand-let) ((let*) expand-let*)
    ((letrec*) expand-letrec-star) ((letrec) (lambda (e m s r) (expand `(letrec* ,@(cdr e)) m s r)))
...
(define (strip-suffix str)
  (let ((len (string-length str)))
    (let loop ((i (- len 1)))
      (if (>= i 0) (if (char=? (string-ref str i) #\.) (if (< i (- len 1))
                  (let check-numeric ((j (+ i 1))) (if (and (< j len) (char-numeric? (string-ref str j))) (check-numeric (+ j 1)) (if (= j len) (strip-suffix (substring str 0 i)) str)))
                  str) (loop (- i 1))) str))))
