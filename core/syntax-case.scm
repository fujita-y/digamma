...
(define (collect-pattern-vars pattern literals ellipsis)
  (cond
    ((and (symbol? pattern)
     ...
     (list pattern))
    ((and (pair? pattern) (eq? (car pattern) ellipsis)) '())
    ((pair? pattern)
  ...
  (cond
    ...
    ((and (pair? template) (eq? (car template) ellipsis))
     (if (and (pair? (cdr template)) (null? (cddr template)))
         (expand-syntax (cadr template) bindings context meta-env depth (gensym) literals suffix)
         (let ((new-sym (rename-symbol ellipsis suffix)))
           (register-renamed! new-sym ellipsis context)
           new-sym)))
