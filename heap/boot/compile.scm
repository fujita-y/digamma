               ...
               (lambda (ht)
                 (cond ((core-hashtable-ref (current-source-comments) form #f)
                        ...
                        (lambda (datum)
                          (if (< datum 0)
                              `(,form
                                ,(core-hashtable-ref (current-source-comments) '|.&SOURCE-PATH| #f)
                                unquote
                                (- datum))
                              `(,(core-hashtable-ref (current-source-comments) '|.&SOURCE-PATH| #f) unquote datum))))
                       (else (list form)))))
              (else (list form)))
        '())))
  ...
  (lambda (form cte discard tail)
    (define compile-anonymous
      (lambda (form cte tail comment)
        (if tail
            `(,@(compile-argument-each (cdr form) cte) (ret.subr.gloc.of ,(car form) unquote comment))
            `(,@(compile-argument-each (cdr form) cte)
              (subr.gloc.of ,(car form) ,(length (cdr form)) unquote comment)))))
                   ...
                   (if tail
                       ...
                       (if (iloc? (cadr form) cte)
                           (cond (discard '())
                                 (else
                                   (cond ((eq? subr car)
                                          `((car.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                                         ((eq? subr cdr)
                                          `((cdr.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                                         ((eq? subr cadr)
                                          `((cadr.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                                         ((eq? subr cddr)
                                          `((cddr.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                                         (else (compile-anonymous form cte tail comment)))))
                              ...
                              (let ((opd1 (cadr form)) (opd2 (caddr form)))
                                (let-values (((lhs rhs)
                                              (cond ((and (fixnum? opd1) (symbol? opd2) (iloc? opd2 cte))
                                                    ...
                                                    (else (values #f #f)))))
                                  (if lhs
                                      `((n+.iloc ,(make-iloc-operand lhs cte) ,rhs unquote comment))
                                      (compile-anonymous form cte tail comment)))))
                             ((eq? subr -)
                              (let ((opd1 (cadr form)) (opd2 (caddr form)))
                                (if (and (fixnum? opd2) (symbol? opd1) (iloc? opd1 cte))
                                    `((n+.iloc ,(make-iloc-operand opd1 cte) ,(- opd2) unquote comment))
                                    (compile-anonymous form cte tail comment))))
                             (else (compile-anonymous form cte tail comment)))))
                  (else (compile-anonymous form cte tail comment)))
            (compile-anonymous form cte tail comment))))))
  ...
  (lambda (form cte)
    (define compile-anonymous
      (lambda (form cte comment)
        `(,@(compile-argument-each (cdr form) cte)
          (push.subr.gloc.of ,(car form) ,(length (cdr form)) unquote comment))))
    (let ((comment (make-application-comment form)))
      (let ((subr (top-level-value (car form))))
        (if (core-hashtable-contains? ht-special-subr-argument subr)
            (cond ((null? (cdr form))
                  ...
                  ((null? (cddr form))
                   (if (iloc? (cadr form) cte)
                       (cond ((eq? subr car) `((push.car.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                             ((eq? subr cdr) `((push.cdr.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                             ((eq? subr cadr) `((push.cadr.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                             ((eq? subr cddr) `((push.cddr.iloc ,(make-iloc-operand (cadr form) cte) unquote comment)))
                          ...
                          (let ((opd1 (cadr form)) (opd2 (caddr form)))
                            (let-values (((lhs rhs)
                                          (cond ((and (iloc? opd1 cte) (fixnum? opd2)) (values opd1 opd2))
                                                ...
                                                (else (values #f #f)))))
                              (if lhs
                                  `((push.n+.iloc ,(make-iloc-operand lhs cte) ,rhs unquote comment))
                                  (compile-anonymous form cte comment)))))
                         ((eq? subr -)
                          (let ((opd1 (cadr form)) (opd2 (caddr form)))
                            (if (and (iloc? opd1 cte) (fixnum? opd2))
                                `((push.n+.iloc ,(make-iloc-operand opd1 cte) ,(- opd2) unquote comment))
                                (compile-anonymous form cte comment))))
                         (else (compile-anonymous form cte comment))))
                  (else (compile-anonymous form cte comment)))
            (if (null? (cdr form))
  ...
  (lambda (form cte)
    (cond ((pair? form)
           (cond ((top-level-subr (car form)) (compile-subr-argument form cte))
                 ...
                 ((eq? (car form) 'quote) `((push.const unquote (cadr form))))
                 ...
                 (else
                   (if (iloc? form cte)
                       (let ((opd (make-iloc-operand form cte)))
                         (case (car opd)
                               ((0) `((push.iloc.0 unquote (cdr opd))))
                               ((1) `((push.iloc.1 unquote (cdr opd))))
                               (else `((push.iloc ,@opd)))))
                       `((push.gloc.of ,form))))))
          (else `((push.const unquote form))))))
          ...
          ((symbol? proc)
           (cond ((iloc? proc cte)
                  (if (core-hashtable-contains? (ht-local-closures) proc)
                      `((apply.iloc+ ,(make-iloc-operand proc cte) unquote comment))
                      `((apply.iloc ,(make-iloc-operand proc cte) unquote comment))))
                 ((top-level-subr proc) `((ret.subr.gloc.of ,proc unquote comment)))
                 (else `((apply.gloc.of ,proc unquote comment)))))
          (else `((const unquote proc) (apply ,@comment))))))
  ...
  (lambda (form cte discard tail)
    (let ((body (compile-expression (caddr form) cte #f #f)) (retval (if tail (list (list 'ret.const.unspec)) '())))
      (cond ((iloc? (cadr form) cte)
             `(,@body (set.iloc ,(make-iloc-operand (cadr form) cte) unquote (make-application-comment form)) ,@retval))
            (else
              (let ((touch (if (backtrace) (compile-touch (cadr form) cte) '())))
                `(,@body ,@touch (set.gloc.of ,(cadr form) unquote (make-application-comment form)) ,@retval)))))))
              ...
              (let-values (((front back)
                ...
                (let ((cte (cte-extend-iloc (map car bindings) cte)))
                  (let ((front-code (compile-argument-each (map cadr front) cte))
                    ...
                    (let ((code
                            (if (null? front-code)
                                `((extend.unbound unquote (length bindings)) ,@back-code ,@body-code)
                                `((extend.unbound unquote (length bindings))
                                  ,@front-code
                                  (enclose unquote (length front-code))
                                  ,@back-code
                                  ,@body-code))))
                      (if tail code (list (cons 'call code))))))))))))
                ...
                (if (and (symbol? (cadr form)) (eq? (cadr form) (caddr form)))
                    ...
                    (cond (tail
                            (if (and (null? (cdr true-code)) (eq? (caar true-code) 'ret.const))
                                `(,@cond-code (if.true.ret.const unquote (cdar true-code)) ,@false-code)
            ...
            (cond ((and (eq? inst-ret-const 'if.null?.ret.const) (symbol? arg) (eq? arg true))
                  ...
                  (else
                    (if (and (null? (cdr true-code)) (memq (caar true-code) '(const ret.const)))
                        (cond (tail `(,@arg-code (,inst-ret-const unquote (cdar true-code)) ,@false-code))
                ...
                (cond (discard '())
                      ...
                      ((symbol? form)
                       (if (iloc? form cte)
                           (let ((opd (make-iloc-operand form cte)))
                             (case (car opd)
                                   ((0) `((iloc.0 unquote (cdr opd))))
                                   ((1) `((iloc.1 unquote (cdr opd))))
                                   (else `((iloc ,@opd)))))
                           `((gloc.of ,form))))
                      (else `((const unquote form)))))))))
