                  ...
                  (let loop ((clauses clauses) (exports '()) (imports '()) (depends '()) (commands '()))
                    (if (null? clauses)
                        ...
                        (destructuring-match clauses
                          ...
                          ((('include path ...) more ...)
                           (every1 string? path)
                           (loop `((begin ,@(apply append (map (lambda (e) (read-include-file library-name e 'include)) path))) ,@more)
                                 exports imports depends commands))
                          ((('include-ci path ...) more ...)
                           (every1 string? path)
                           (loop `((begin ,@(apply append (map (lambda (e) (read-include-file library-name e 'include-ci)) path))) ,@more)
                                 exports imports depends commands))
                          ((('include-library-declarations path ...) more ...)
                           (every1 string? path)
                           (loop (append (apply append (map (lambda (e) (read-include-file library-name e 'include-library-declarations)) path)) more)
                                 exports imports depends commands))
