       ...
       (let ((library-id (library-name->id form library-name))
         ...
         (parameterize ((current-include-files (make-core-hashtable)))
           (let ((coreform
                           ...
                           ((('include path ...) more ...)
                            ...
                            (let ((more
                                    `((begin
                                        ,@(apply
                                            append
                                            (map (lambda (e) (read-include-file library-name e 'include)) path)))
                                      ,@more)))
                           ...
                           ((('include-ci path ...) more ...)
                            ...
                            (let ((more
                                    `((begin
                                        ,@(apply
                                            append
                                            (map (lambda (e) (read-include-file library-name e 'include-ci)) path)))
                                      ,@more)))
                              (loop more exports imports depends commands)))
                            ...
                            (let ((more
                                    (append
                                      (apply
                                        append
                                        (map (lambda (e)
                                               (read-include-file library-name e 'include-library-declarations))
                                             path))
                                      more)))
                              (loop more exports imports depends commands)))
             ...
             coreform))))
      (_
        (syntax-violation 'define-library "expected library name and declarations" (abbreviated-take-form form 4 8))))))
