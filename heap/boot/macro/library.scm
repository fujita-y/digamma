                      ...
                      (let ((e2 (assq id (reverse defs))))
                        (cond ((eq? e1 e2)
                                (let ((e2 (assq id macros)))
                                  (syntax-violation who "duplicate definitions"
                        ...
                        (cond ((eq? e1 e2)
                              ...
                              (else
                                (syntax-violation who "duplicate definitions"
                                                  ...
                                                  (annotate `(define-syntax ,(car e2) ...) e2)))))))
                (else
                  (syntax-violation who "duplicate definitions" id)))))))
  ...
  (lambda (form library-id library-version body exports imports depends env libenv)
    ...
    (define rewrite-body
      (lambda (body defs macros renames)
        (rewrite-library-body form library-id library-version body defs macros renames exports imports depends env libenv)))
