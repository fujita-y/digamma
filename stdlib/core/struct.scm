      ...
      ((?_ ?struct-name ?field-names)
       (let ((struct-name (syntax->datum #'?struct-name))
         ...
         (let ((make-name (string->symbol (format "make-~a" struct-name)))
               ...
               (desc-name (string->symbol (format "struct:~a" struct-name)))
               (field-count (length field-names)))
           (let ((field-refs (iota field-count 1)))
             (with-syntax
                 ((?make-name (datum->syntax #'?_ make-name))
                  (?pred-name (datum->syntax #'?_ pred-name))
                  (?make-rules
                    (datum->syntax
                      #'k
                      `(syntax-rules ()
                  ...
                  ((?getter-rules ...)
                   (map (lambda (field-ref)
                          (datum->syntax
                            #'k
                            `(syntax-rules () ((_ obj) (tuple-ref obj ,field-ref)))))
                        field-refs))
                  ((?setter-rules ...)
                   (map (lambda (field-ref)
                          (datum->syntax
