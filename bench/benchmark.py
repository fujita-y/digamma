...
# --- Configuration ---
IMPLEMENTATIONS = {
    "nanos":   {"command": ["make", "nanos"],   "label": "Digamma"},
...
def trimmed_mean(values):
    """Return the mean after excluding the single min and max value.

    Falls back to a regular mean when there are fewer than 3 samples so
    that trimming never leaves an empty list.
    """
    if len(values) >= 3:
...
def collect_data():
    """Run all implementations NUM_RUNS times and compute trimmed averages."""
    ...
    # Compute trimmed averages (min/max excluded): averages[impl][bench] = {"real": avg, "user": avg}
    averages = {}
    for impl_key in IMPLEMENTATIONS:
        averages[impl_key] = {}
        for bench_name, metrics in all_runs[impl_key].items():
            averages[impl_key][bench_name] = {
                "real": trimmed_mean(metrics["real"]),
                "user": trimmed_mean(metrics["user"]),
            }
...
<body>
    <header class="page-header">
        ...
        <p class="subtitle">Gambit Benchmarks &mdash; Real / User Time (seconds, lower is better) &mdash; Trimmed average of {NUM_RUNS} runs (min/max excluded) &mdash; {timestamp}</p>
    </header>
    ...
    <footer class="footer">
        Generated by benchmark.py &mdash; {NUM_RUNS} runs, trimmed average (min/max excluded) &mdash; {timestamp}
    </footer>
</body>
</html>"""
