...
void test_printf_with_scheme_objects() {
  ...
  printer.printf("list: %w", list);
  assert(ss.str() == "list: (1 2 3)");
  std::cout << "%w with list test passed" << std::endl;

  // Test multiple %w
  ss.str("");
  printer.printf("%w + %w = %w", make_fixnum(1), make_fixnum(2), make_fixnum(3));
  assert(ss.str() == "1 + 2 = 3");
  std::cout << "Multiple %w test passed" << std::endl;

  // Test mixing %w with standard directives
  ss.str("");
  printer.printf("The answer is %w which is %d in decimal", make_fixnum(42), 42);
