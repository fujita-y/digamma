...
int main() {
  ...
  // nanos_init_llvm(); // Seems implicit or handled by codegen constructor in tests?
  // In test_codegen.cpp, llvm::InitializeNativeTarget() is called manually.
  ...
  scm_obj_t scm_subr_list = my_make_closure((void*)subr_list, 0, 1, 1, nullptr, scm_nil, 1);
  c_global_set(make_symbol("list"), scm_subr_list);

  scm_obj_t scm_subr_apply = my_make_closure((void*)subr_apply, 0, 1, 1, nullptr, scm_nil, 1);
