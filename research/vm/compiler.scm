...
(define (analyze-mutated-vars expr rec-vars)
  (cond
    ((pair? expr)
     (case (car expr)
       ((quote) '())
       ((set!) (if (memq (cadr expr) rec-vars)
                   (analyze-mutated-vars (caddr expr) rec-vars)
                   (cons (cadr expr) (analyze-mutated-vars (caddr expr) rec-vars))))
       ((lambda) (let loop ((body (cddr expr))) (if (null? body) '() (set-union (analyze-mutated-vars (car body) rec-vars) (loop (cdr body))))))
       (else (let loop ((exprs expr)) (if (null? exprs) '() (set-union (analyze-mutated-vars (car exprs) rec-vars) (loop (cdr exprs))))))))
    (else '())))
...
(define (compile expr)
  (let* ((rec-vars '())
         (_ (let walk ((e expr))
              (if (match-rec-pattern e) (set! rec-vars (cons (car (car (cadr e))) rec-vars)))
              (if (pair? e) (if (eq? (car e) 'quote) '() (for-each walk e)))))


         (mutated (analyze-mutated-vars expr rec-vars))
         (labels 0)
         (closure-labels 0)
         (compiled-code '())
         (all-closure-code '()))
